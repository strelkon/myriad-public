...
for calibration_date=start_calibration_date:calquarters:end_calibration_date
    ...
    zeta_b=0.5;
    pi_star=.005;
    % [alpha_pi_EA,beta_pi_EA,sigma_pi_EA,epsilon_pi_EA]=estimate(diff(log(eu.gdp_deflator_quarterly(T_estimation_exo-1:T_calibration_exo))));
    % [alpha_Y_EA,beta_Y_EA,sigma_Y_EA,epsilon_Y_EA]=estimate(log(eu.real_gdp_quarterly(T_estimation_exo:T_calibration_exo)));
    % [rho,r_star,xi_pi,xi_gamma,pi_star]=estimate_taylor_rule((eu.euribor(T_estimation_exo:T_calibration_exo)+1).^(1/4)-1,exp(diff(log(eu.gdp_deflator_quarterly(T_estimation_exo-1:T_calibration_exo))))-1,exp(diff(log(eu.real_gdp_quarterly(T_estimation_exo-1:T_calibration_exo))))-1);
    
    % [alpha_C_E,beta_C_E,epsilon_C_E]=estimate(log(timescale*sum(exports-reexports)*eu.real_exports_quarterly(T_estimation_exo:T_calibration_exo)/eu.real_exports_quarterly(T_calibration_exo)));
    [alpha_gamma_E,beta_gamma_E,epsilon_gamma_E]=estimate(diff(log(eu.real_exports_quarterly(T_estimation_exo-1:T_calibration_exo))));
    [alpha_pi_E,beta_pi_E,epsilon_pi_E]=estimate(diff(log(eu.exports_deflator_quarterly(T_estimation_exo-1:T_calibration_exo))));
    % [alpha_Y_I,beta_Y_I,epsilon_Y_I]=estimate(log(timescale*sum(imports)*eu.real_imports_quarterly(T_estimation_exo:T_calibration_exo)/eu.real_imports_quarterly(T_calibration_exo)));
    [alpha_gamma_I,beta_gamma_I,epsilon_gamma_I]=estimate(diff(log(eu.real_imports_quarterly(T_estimation_exo-1:T_calibration_exo))));
    [alpha_pi_I,beta_pi_I,epsilon_pi_I]=estimate(diff(log(eu.imports_deflator_quarterly(T_estimation_exo-1:T_calibration_exo))));
    ...
    for g=1:length(geos)
        ...
        % [alpha_C_G(g),beta_C_G(g),epsilon_C_G(:,g)]=estimate(log(timescale*sum(government_consumption(:,g))*data.real_government_consumption_quarterly(T_estimation_exo:T_calibration_exo)/data.real_government_consumption_quarterly(T_calibration_exo)));
        [alpha_gamma_G(g),beta_gamma_G(g),epsilon_gamma_G(:,g)]=estimate(diff(log(data.real_government_consumption_quarterly(T_estimation_exo-1:T_calibration_exo))));
        [alpha_pi_G(g),beta_pi_G(g),epsilon_pi_G(:,g)]=estimate(diff(log(data.government_consumption_deflator_quarterly(T_estimation_exo-1:T_calibration_exo))));
    end
    ...
    % Initial conditions
    Y=timescale*sum(sum(output))*eu.real_gdp_quarterly(T_estimation_exo:min(T_calibration_exo+T,T_calibration_exo_max))/eu.real_gdp_quarterly(T_calibration_exo);
    gamma=diff(log(eu.real_gdp_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    pi=diff(log(eu.household_consumption_deflator_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    P=eu.household_consumption_deflator_quarterly(T_estimation_exo:min(T_calibration_exo+T,T_calibration_exo_max))/eu.household_consumption_deflator_quarterly(T_calibration_exo);
    ...
    gamma_E=diff(log(eu.real_exports_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    C_E=timescale*sum(exports-reexports)*exp(cumsum(gamma_E))/exp(sum(gamma_E(1:T_prime)));
    pi_E=diff(log(eu.exports_deflator_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    P_E=exp(cumsum(pi_E))/exp(sum(pi_E(1:T_prime)));
    gamma_I=diff(log(eu.real_imports_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    Y_I=timescale*sum(imports)*exp(cumsum(gamma_I))/exp(sum(gamma_I(1:T_prime)));
    pi_I=diff(log(eu.imports_deflator_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
    P_I=exp(cumsum(pi_I))/exp(sum(pi_I(1:T_prime)));
    ...
    for g=1:length(geos)
        ...
        Y_f(:,g)=timescale*sum(output(:,g))*data.real_gdp_quarterly(T_estimation_exo:T_calibration_exo)/data.real_gdp_quarterly(T_calibration_exo);
        gamma_f(:,g)=diff(log(data.real_gdp_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
        pi_f(:,g)=diff(log(data.household_consumption_deflator_quarterly(T_estimation_exo-1:T_calibration_exo)));
        P_f(:,g)=data.household_consumption_deflator_quarterly(T_estimation_exo:min(T_calibration_exo+T,T_calibration_exo_max))/data.household_consumption_deflator_quarterly(T_calibration_exo);
        gamma_G(:,g)=diff(log(data.real_government_consumption_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
        C_G(:,g)=timescale*sum(government_consumption(:,g))*exp(cumsum(gamma_G(:,g)))/exp(sum(gamma_G(1:T_prime,g)));
        pi_G(:,g)=diff(log(data.government_consumption_deflator_quarterly(T_estimation_exo-1:min(T_calibration_exo+T,T_calibration_exo_max))));
