...
for g=1:G
    ...
    for i=1:I
        DM_d_ig(i)=a_sg(g,G_i(i),F_i(i)).*DM_d_i(i);
    end
    DM_nominal_ig=zeros(size(DM_d_ig));
    I_g=find(DM_d_ig>0);
    S_fg=S_f;
    S_fg_=S_f_;
    F_g=find(G_f==g&S_fg>0);
...
%         F_g=sample(F_g,length(I_g),pr_f);
        ...
        F_g=find(G_f==g&S_fg>eps);
    end
    if ~isempty(I_g)
        DM_d_ig_=DM_d_ig;
        I_g=find(DM_d_ig_>0);
        F_g=find(G_f==g&S_fg_>0);
        while ~isempty(I_g) && ~isempty(F_g)
            I_g=shuffle(I_g);
%             pr_price_f=max(0,exp(-2*P_f(F_g))./sum(exp(-2*P_f(F_g))));
%             pr_size_f=S_f(F_g)/sum(S_f(F_g));
...
%             F_g=sample(F_g,length(I_g),pr_f);
            ...
            F_g=find(G_f==g&S_fg_>eps);
        end
    end
    
    for i=1:I
        DM_i(i)=a_sg(g,G_i(i),F_i(i)).*DM_d_i(i)-DM_d_ig(i)+DM_i(i);
    end
    P_bar_i=P_bar_i+DM_nominal_ig;
    
    I_d_ig=b_CF_g(g,F_i).*I_d_i;
    I_nominal_ig=zeros(size(I_d_i));
    C_d_hg=[b_HH_g(g,F_h).*C_d_h+b_CFH_g(g,F_h).*I_d_h,c_E_g(g)*C_d_l,c_G_g(g,:).*C_d_j];
...
%             F_g=sample(F_g,length(H_g),pr_f);
    ...
    I_i=I_i+b_CF_g(g,F_i).*I_d_i-I_d_ig;
    P_CF_i=P_CF_i+I_nominal_ig;
    
    C_h=b_HH_g(g,F_h).*C_d_h-max(0,C_d_hg(1:H)-b_CFH_g(g,F_h).*I_d_h)+C_h;
    I_h=max(0,b_CFH_g(g,F_h).*I_d_h-C_d_hg(1:H))+I_h;
    C_j=c_G_g(g,:).*C_d_j-C_d_hg(H+L+1:H+L+J)+C_j;
    C_l=sum(c_E_g(g)*C_d_l)-sum(C_d_hg(H+1:H+L))+C_l;
    
    P_bar_h=P_bar_h+max(0,C_real_hg(1:H).*(C_d_h.*b_HH_g(g,F_h)-max(0,C_d_hg(1:H)-b_CFH_g(g,F_h).*I_d_h))./(C_d_h.*b_HH_g(g,F_h)+b_CFH_g(g,F_h).*I_d_h-C_d_hg(1:H)));
    P_bar_CF_h=P_bar_CF_h+max(0,C_real_hg(1:H).*(max(0,b_CFH_g(g,F_h).*I_d_h-C_d_hg(1:H)))./(C_d_h.*b_HH_g(g,F_h)+b_CFH_g(g,F_h).*I_d_h-C_d_hg(1:H)));
